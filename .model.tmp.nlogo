  ...
  ask people with [escaping = true and health_state > 0 and panic = true][
    ...
    if not dead [
      ...
      ifelse (rational = true)
        [move_forward set panic false]; assumption: once the person acts in a rational way then panic effect vanishes, this also help to ensure that the overall panic effect won't last forever
        [follow_crowd]
    ]
  ]
...
ask people[
    ...
    ifelse (panic = false)[
      ...
      face destination]
  [set destination max-one-of neighbors with [pcolor = white] [num_people] ]; if panic is present people will face the neihbor with most people in
  ]
end
